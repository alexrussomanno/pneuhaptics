...
int gameMode = 2;
unsigned long rampInterval = 750; // number of millisecs in each ramp steps (500 is the standard)
...
const int activateButton[] = {500,500,500,500};  // this is the minimum register value when a button is pressed. This value is based on analogRead of the FSR_PIN
const int deactivateButton[] = {80,80,80,80};
unsigned long flutter = 100;   // the rate at which the fourth dot of led or valve turns on and off
unsigned long cushionTime = 100; // the time in millis a user is allowed to be early before it beeps 
float blinkOn = .8;
boolean beeper = LOW;
boolean reward = HIGH;
...
unsigned long maxWaitTime = 4000;
...
//    else if (buttonStatus(x)==0 && currentMillis > previousMillis[x] + waitTime[x] + (6+blinkOn)*rampInterval){
  ...
  if(analogRead(FSR_PIN[x]) > activateButton[x] && toggle[x] == 0 || no_response[x]) {   
    ...
    if(reward==LOW){
      ...
      if(currentMillis < previousMillis[x]+waitTime[x]+ (4+blinkOn)*rampInterval+cushionTime && currentMillis > previousMillis[x]+waitTime[x]+ (4+blinkOn)*rampInterval - cushionTime){
        ...
        if(beeper == HIGH){
          // Beep at user for being too or too late
